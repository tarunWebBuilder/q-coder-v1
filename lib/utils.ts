...
export function extractAllCodeBlocks(input: string): Array<{
  ...
  while ((match = codeBlockRegex.exec(input)) !== null) {
    ...
    let { language, path } = parseFenceTag(fenceTag);

    ({ language, path } = normalizePathForJsx(path, language, code));
...
function normalizePathForJsx(
  path: string,
  language: string,
  code: string,
): { path: string; language: string } {
  if (!containsJsx(code)) return { path, language };

  const lowerPath = path.toLowerCase();
  if (lowerPath.endsWith(".ts")) {
    return { path: path.replace(/\.ts$/i, ".tsx"), language: "tsx" };
  }
  if (lowerPath.endsWith(".js")) {
    return { path: path.replace(/\.js$/i, ".jsx"), language: "jsx" };
  }
  if (!path.includes(".")) {
    if (language.toLowerCase() === "ts") {
      return { path: `${path}.tsx`, language: "tsx" };
    }
    if (language.toLowerCase() === "js") {
      return { path: `${path}.jsx`, language: "jsx" };
    }
  }
...
function containsJsx(code: string): boolean {
  const selfClosing = /<[A-Za-z][^>]*\/>/.test(code);
  const paired = /<[A-Za-z][^>]*>[\s\S]*<\/[A-Za-z][^>]*>/.test(code);
